...
def grant_reward_points_after_evaluate(request, semester, **_kwargs):
    ...
    if granting:
        ...
        elif Evaluation.objects.filter(participants=request.user, course__semester=semester, is_rewarded=True).exclude(state__in=[Evaluation.State.EVALUATED, Evaluation.State.REVIEWED, Evaluation.State.PUBLISHED], voters=request.user).exists():
